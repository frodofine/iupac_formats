...
int main(int argc, char* argv[])
{
	try {
		...
		// Load reactant(s).
		rc = rxn.add_reactant();
		mol_reader.read_molecule("reactant_01.mol", *rc);
		// Load product(s).
		rc = rxn.add_product();
		mol_reader.read_molecule("product_01.mol", *rc);
		// Load agent(s).
		rc = rxn.add_agent();
		mol_reader.read_molecule("agent_01.mol", *rc);

		rc = rxn.add_agent();
		mol_reader.read_molecule("agent_02.mol", *rc);
