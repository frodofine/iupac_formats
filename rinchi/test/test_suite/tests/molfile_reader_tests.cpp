...
{
	std::ifstream sdfile ("InChI_TestSet.sdf");
	std::ifstream expected_data ("its-std-01.txt");
	...
	if (!expected_data)
		throw rinchi::unit_test::TestFailure("Can't open InChI standard test results 'its-std-01.txt'.");
	...
	while (sdfile) {
		...
		if (molfile.str().length() >= 5) {
			...
			// Uncomment to aid debugging if a molecule fails the test:
			//std::cout << "    #" << mol_no << ", ending at line " << current_line_no << "\r";
			mol_no++;
		}
	}
